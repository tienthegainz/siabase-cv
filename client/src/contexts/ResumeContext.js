...
const ResumeProvider = ({ children }) => {
  const { updateResume, debouncedUpdateResume } = useContext(DatabaseContext);
  // const { updateResume, debouncedUpdateResume } = useContext(
  //   LocalResumeContext
  // );
  ...
  const reducer = useCallback((state, { type, payload }) => {
    ...
    switch (type) {
      ...
      case 'reset_data':
        ...
        newState.user = temp.user;
        newState.name = temp.name;
        newState.preview = templatePreviews.onyx;
        newState.createdAt = temp.createdAt;
        debouncedUpdateResume(newState);
        return newState;

      case 'load_demo_data':
        newState = { ...state, ...demoState };
        newState.metadata.layout = demoState.metadata.layout;
        debouncedUpdateResume(newState);
        return newState;

      case 'on_upload_to_skynet':
        newState = {
          ...state,
          skynet: { isSyncedWithSkynet: true, skylink: payload.skylink }
        };
        debouncedUpdateResume(newState);
        return newState;

      case 'update_skynet_synced_status':
        if (state.skynet.isSyncedWithSkynet) {
          newState = {
            ...state,
            skynet: { ...state.skynet, isSyncedWithSkynet: false }
          };
          updateResume(newState);
          return newState;
        }
