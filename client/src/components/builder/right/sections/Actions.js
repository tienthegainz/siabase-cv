...
const Actions = ({ id }) => {
  ...
  const handleUpload = () => emitter.emit(events.SKYNET_UPLOAD_MODAL);
  ...
  const getSharableUrl = () => {
    ...
    const BASE_URL =
      process.env.NODE_ENV === 'development'
        ? 'http://localhost:8000'
        : 'https://siabase-cv.web.app';

    return `${BASE_URL}/r/${shareId}`;
  };
  ...
  return (
    <section>
      ...
      <div className={styles.container}>
        <h5>{t('builder.actions.skynetUpload.button')}</h5>

        <p className='leading-loose'>
          {t('builder.actions.skynetUpload.text')}
        </p>

        <div className='mt-4 flex'>
          <Button onClick={handleUpload}>
            {t('builder.actions.skynetUpload.button')}
          </Button>
        </div>
      </div>
